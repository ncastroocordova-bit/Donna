...
- **Sombrero Louis (plata)** — planilla `GOOGLE_SHEET_ID_LOUIS`: finanzas + estados de cuenta. Hojas: Transacciones, Categorias, Tarjetas y Deuda, Dashboard, Comparativo, Metas, Compras_Detalle, Deuda_Mensual, Saldos.
...
- **Finanzas:** deuda real **incluye la línea**. Faro: Deuda total real ~**$2.297.966**, Intereses muertos ~**$63.908/mes** — **ambas son datos vivos de la planilla** (Finanzas v4 corrigió la deuda del $2.028.091 histórico; Finanzas v5/Ola 1 corrigió el interés: el estado de cuenta manda sobre el cálculo tasa×rotativa cuando lo reporta, ver `Tarjetas y Deuda` C25/C37). *(No hardcodear ninguna de las dos cifras como assert de eval; el eval verifica que el faro calce con `Tarjetas y Deuda` B4:B8.)* **v2:** intención del gasto (Necesario/Inversión/Deseo en `Transacciones`, se confirma en el digest) + metas financieras con progreso (tab `Metas`, sin input diario). **v5 (reconciliación de estados, 2026-07-23):** cada estado/cartola que llega se compara contra `Transacciones` — compras del documento vs. anotado por Donna, con las compras que faltan yendo al mismo buffer del digest (chips, toque para confirmar). Cubre tarjeta de crédito **y** cuenta corriente (débito); esta última también trae **ingresos** (abonos) y el **saldo de cierre mensual** (hoja `Saldos`, no `Deuda_Mensual`). **No** se agregan cuentas con saldos en vivo / doble-entrada que Nico deba mantener (rompe "registro sin fricción") — **excepción**: el saldo mensual de v5 sale gratis del documento que Donna ya lee para reconciliar, cero input extra, así que se guarda sin toque (mismo criterio que el faro).
- **Regla del RUT propio (transversal a Finanzas):** un movimiento entre cuentas del propio Nico —transferencia saliente o abono entrante— **no es gasto ni es ingreso**, es traslado; solo cuenta lo que cruza hacia/desde un tercero. Se reconoce por RUT cuando el banco lo trae, y por **nombre** (config `DUENO_NOMBRES`, coma-separado) cuando no — comparado por subconjunto de palabras, no por igualdad exacta, porque las cartolas envuelven el nombre en su propia glosa ("TRASPASO DE: NICOLAS EMILIO CASTRO INTERNET").
- **Captura de compras (Finanzas v3, alimenta a Compras):** la boleta se lee **ítem por ítem** (foto → ítem+precio+total) y va a `Compras_Detalle`. **`Transacciones` es una fila por movimiento del banco (para reconciliar); `Compras_Detalle` es el detalle por ítem (de ahí salen todas las métricas de gasto por categoría) — nunca se mezclan.** Toda transacción de gasto deja **al menos una línea** en `Compras_Detalle` (sin desglose = una línea por el monto completo); las líneas de un mismo `ID_Tx` siempre suman el monto de su transacción padre. **Foto/dictado y correo del mismo gasto se correlacionan por monto+fecha(+comercio) → una sola transacción, jamás doble conteo** (el correo es el total canónico; la foto/dictado aporta los ítems) — la correlación corre en dos pasadas: contra el buffer del día y, si ya se escribió, contra lo ya registrado en la planilla. Para **comercios "de compras"** (súper, almacén, San Valentín) sin detalle, Donna pregunta **al momento** "¿qué compraste?" → foto o desglose por categoría ("2000 chanchería, resto pan", el resto cuadra al total); es prompt **transaccional**, no cuenta contra el tope 1/día. Cada línea se marca `Predecible` (sí = despensa/reposición; no = perecible/cotidiano) y **solo `Predecible=sí` alimenta el predictor de Compras Fase 2**. La marca sale de palabras clave, pero **el chip 📦/🥖 del digest manda sobre ellas y se aprende**: la corrección se persiste en Supabase (`items_predecibles`, migración 015) y se consulta antes de la heurística — Nico no corrige dos veces el mismo ítem (canon de la espina: el aprendizaje va a Supabase, no se vuelve a inferir desde cero).
